...
if [[ -n "$NOSTR_CONFIGURED" ]]; then
    ...
    # CRITICAL: chown agent dir + create seed env AS $INFRA_USER BEFORE running
    # `nostr.mjs login`. Otherwise the file is root:600 and sudo login can't
    # read/rewrite it -- login silently fails and the generated NSEC/NPUB
    # never land in the env file.
    chown "$INFRA_USER:fagent" "$agent_dir"
    chmod 700 "$agent_dir"
    sudo -Hu "$INFRA_USER" tee "$agent_dir/nostr.env" >/dev/null <<NOSTREOF
...
NOSTREOF
    ...
    # Pre-flight: require fagents-cli + nostr.mjs to actually exist.
    ...
    if [[ -z "$nostr_setup_failed" ]]; then
        ...
        # Post-install verification: npm could exit 0 but not install the
        # declared packages (stale package.json, dry-run, etc.).
        if [[ -z "$nostr_setup_failed" ]]; then
            if [[ ! -f "$CLI_DIR/node_modules/nostr-tools/package.json" ]] || \
               [[ ! -f "$CLI_DIR/node_modules/ws/package.json" ]]; then
    ...
    if [[ -z "$nostr_setup_failed" ]]; then
        ...
        if ! login_out=$(sudo -Hu "$INFRA_USER" "$CLI_DIR/nostr.mjs" --env-file "$agent_dir/nostr.env" login "${login_args[@]}" 2>/dev/null); then
            log_warn "Nostr login failed -- run manually: sudo -Hu $INFRA_USER $CLI_DIR/nostr.mjs --env-file $agent_dir/nostr.env login"
            nostr_setup_failed=1
        else
    ...
    # the filenames are visible to fagent-group members.
    chmod 750 "$agent_dir"
    chmod 600 "$agent_dir/nostr.env"
    ...
    if [[ -n "$nostr_setup_failed" ]]; then
        NOSTR_CONFIGURED=""
        # Remove the partial nostr.env (relays only, no NOSTR_NSEC) so the
        # daemon's grep-for-NSEC guard sees a clean "not configured" state.
    ...
    for user in "${CODEX_AGENTS[@]}"; do
        ...
        case "$CODEX_AUTH_MODE" in
            oauth)
                ...
                sudo -Hu"$user" bash -lc "CODEX_HOME='$codex_home' bash" <<'WRITE_CODEX_MODEL_OVERRIDE_BODY'
