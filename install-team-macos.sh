...
# ── X (Twitter) config (collected upfront, installed in Step 5c) ──
enable_x=""
if [[ -z "${NONINTERACTIVE:-}" ]]; then
    echo ""
...
- Voice output: \`sudo -Hufagents $CLI_DIR/tts-speak.sh <chat-id> "text"\` — text to speech via OpenAI TTS, sent as Telegram voice message
...
# ── Step 5c: X (Twitter) setup (non-interactive — config collected upfront) ──
if [[ ${#X_AGENTS[@]} -gt 0 ]]; then
...
## X (Twitter)
- You have X (Twitter) via \`sudo -Hufagents $CLI_DIR/x.sh\`
