...
cleanup() {
    ...
    launchctl bootout system /Library/LaunchDaemons/ai.fagents-mcp.plist 2>/dev/null || true
    rm -f /Library/LaunchDaemons/ai.fagents-mcp.plist
...
install() {
    ...
    /opt/homebrew/bin/bash "$SCRIPT_DIR/install-team-macos.sh" --skip-claude-auth
...
backend_env_check() {
    ...
    out=$(env NONINTERACTIVE=1 "$key=bogus" \
        ...
        HUMAN_NAMES_INPUT="$HUMAN_NAME" \
        /opt/homebrew/bin/bash "$SCRIPT_DIR/install-team-macos.sh" --skip-claude-auth --comms-port "$COMMS_PORT" 2>&1)
...
verify() {
    ...
    check "$COMMS_USER email.env exists" "test -f /Users/fagents/.agents/$COMMS_USER/email.env"
    email_perms=$(stat -f %A /Users/fagents/.agents/"$COMMS_USER"/email.env 2>/dev/null)
    if [[ "$email_perms" == "600" ]]; then ok "$COMMS_USER email.env is 600"; else not_ok "$COMMS_USER email.env is 600 (got: $email_perms)"; fi
    check "$COMMS_USER email.env has SMTP_HOST from env" "grep -q '^SMTP_HOST=smtp.test.example$' /Users/fagents/.agents/$COMMS_USER/email.env"
    check "$COMMS_USER email.env has SMTP_FROM from env" "grep -q '^SMTP_FROM=$COMMS_USER@test.example$' /Users/fagents/.agents/$COMMS_USER/email.env"
    check "$COMMS_USER email.env has IMAP_HOST from env" "grep -q '^IMAP_HOST=imap.test.example$' /Users/fagents/.agents/$COMMS_USER/email.env"
    check "$COMMS_USER email.env IMAP_USER falls back to SMTP user" "grep -q '^IMAP_USER=$COMMS_USER@test.example$' /Users/fagents/.agents/$COMMS_USER/email.env"
    check "$COMMS_USER email.env IMAP_PASS falls back to SMTP pass" "grep -q '^IMAP_PASS=test-smtp-pw$' /Users/fagents/.agents/$COMMS_USER/email.env"
    check "$COMMS_NAME MEMORY.md has Email Tools note" "grep -q 'Email Tools' /Users/$COMMS_USER/workspace/$COMMS_USER/memory/MEMORY.md"
